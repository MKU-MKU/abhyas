...
export default function PracticePage() {
  ...
  const chapterName = chapters.find((chapter) => chapter.code === chapterKey)?.name
    ?? FALLBACK_CHAPTERS.find(([code]) => code === chapterKey)?.[1]
    ?? "Engineering Survey";
  ...
  useEffect(() => {
    ...
    setChapterKey(valid);
    void startPractice(valid);
    // Initial session only; chapter changes call startPractice directly.
  ...
  function checkAnswer() {
    ...
    try {
      ...
      localStorage.setItem("abhyas:wrong", JSON.stringify(isCorrect
  ...
  return <main className="main"><section style={{ maxWidth: 920, margin: "0 auto" }}>
    ...
    <div style={{ marginTop: 28, display: "flex", justifyContent: "space-between", gap: 16, alignItems: "end", flexWrap: "wrap" }}>
      <div><p className="eyebrow">Real question bank</p><h1>Practice</h1><p className="meta">Verified questions are bundled into the GitHub Pages build. No live Drive file is required for Practice.</p></div>
      <div style={{ minWidth: 300 }}><label className="meta" htmlFor="chapter">Chapter</label><select id="chapter" className="input" value={chapterKey} onChange={(event) => { const value = event.target.value; setChapterKey(value); void startPractice(value); }} disabled={loading}>{(chapters.length ? chapters : FALLBACK_CHAPTERS.map(([code, name]) => ({ code, name, count: 0 }))).map((chapter) => <option key={chapter.code} value={chapter.code}>{chapter.name}{chapter.count ? ` · ${chapter.count} Q` : ""}</option>)}</select></div>
    </div>

    <div className="card" style={{ marginTop: 20, display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}><span className="pill">{chapterName}</span><span className="meta">{questions.length || "—"} questions in this session</span><span className="meta">·</span><span className="meta">Up to 20 per session</span><Link className="cardLink" href="/review" style={{ margin: 0 }}>Review mistakes →</Link><button type="button" className="primaryButton" onClick={() => void startPractice()} disabled={loading} style={{ marginLeft: "auto" }}>{loading ? "Loading…" : "New set ↻"}</button></div>

    {error && <div className="card" style={{ marginTop: 16, borderColor: "var(--danger)" }}><strong>Could not load the question bank.</strong><p>{error}</p><button type="button" className="primaryButton" onClick={() => void startPractice()}>Retry</button></div>}

    {finished && <div className="card" style={{ marginTop: 20, textAlign: "center" }}><p className="eyebrow">Practice complete</p><h2 style={{ fontSize: "2.5rem", marginBottom: 8 }}>{score}/{questions.length}</h2><p className="subtitle" style={{ marginInline: "auto" }}>{questions.length ? Math.round((score / questions.length) * 100) : 0}% accuracy · Great work on {chapterName}.</p><div style={{ display: "flex", justifyContent: "center", gap: 10, flexWrap: "wrap", marginTop: 18 }}><button type="button" className="primaryButton" onClick={() => void startPractice()}>Practice again ↻</button><Link className="secondaryButton" href="/review">Review mistakes →</Link><Link className="secondaryButton" href="/progress">See progress →</Link></div></div>}

    {question && !loading && !finished && <div className="card" style={{ marginTop: 20 }}><div style={{ height: 7, background: "var(--surface-muted)", borderRadius: 99, overflow: "hidden", marginBottom: 22 }}><div style={{ width: `${progress}%`, height: "100%", background: "var(--primary)" }} /></div><div className="meta" style={{ display: "flex", justifyContent: "space-between", gap: 12 }}><span>{chapterName}</span><span>Question {index + 1} / {questions.length}</span></div><h2 style={{ fontSize: "clamp(1.3rem, 3vw, 2rem)", lineHeight: 1.35, marginTop: 12 }}>{question.prompt}</h2><div style={{ display: "grid", gap: 10, marginTop: 24 }}>{question.options.map((option, optionIndex) => <button key={option.id} type="button" className={`questionOption ${optionClass(option.id)}`} onClick={() => !submitted && setSelected(option.id)} disabled={submitted} aria-pressed={selected === option.id}><strong>{String.fromCharCode(65 + optionIndex)}.</strong><span>{option.label}</span></button>)}</div>{submitted && <div className="card" style={{ marginTop: 18, background: "var(--surface-muted)", boxShadow: "none" }}><strong>{correct ? "✓ Correct" : "Review this question"}</strong>{!correct && <p style={{ marginBottom: 6 }}>Correct answer: <strong>{question.options.find((option) => option.id === question.answer)?.label ?? question.answer}</strong></p>}{question.explanation && <p style={{ marginBottom: 0 }}>{question.explanation}</p>}</div>}<div style={{ display: "flex", justifyContent: "space-between", gap: 10, marginTop: 22 }}><button type="button" disabled={index === 0} onClick={() => { setIndex((value) => value - 1); setSelected(null); setSubmitted(false); }} className="secondaryButton">← Previous</button>{!submitted ? <button type="button" onClick={checkAnswer} disabled={selected === null} className="primaryButton" style={{ opacity: selected === null ? .5 : 1 }}>Check answer</button> : <button type="button" onClick={nextQuestion} className="primaryButton">{index < questions.length - 1 ? "Next question →" : "Finish session →"}</button>}</div></div>}
  </section></main>;
}
