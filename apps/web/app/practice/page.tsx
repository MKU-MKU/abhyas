...
const chapterOptions = Array.from(
  new Map(DRIVE_SOURCES.map((source) => [`${source.level}:${source.chapterCode}`, source])).values(),
);
...
export default function PracticePage() {
  ...
  useEffect(() => {
    void startPractice();
    // Initial chapter only; changing chapter is explicit through Start.
  ...
  if (!selectedChapter) {
    return <main className="main"><section className="card"><h1>Question bank is being connected</h1><p>Drive sources are not configured yet.</p></section></main>;
  }
    ...
    <main className="main">
      <section style={{ maxWidth: 920, margin: "0 auto" }}>
        ...
        <div style={{ marginTop: 28, display: "flex", justifyContent: "space-between", gap: 16, alignItems: "end", flexWrap: "wrap" }}>
          <div>
            <p className="eyebrow">Real Question Bank</p>
            <h1>Practice</h1>
            <p className="meta">Questions are loaded from the existing Abhyas Drive files through the V1 content API.</p>
          </div>
          <div style={{ minWidth: 240 }}>
            <label className="meta" htmlFor="chapter">Chapter</label>
            <select id="chapter" className="input" value={chapterKey} onChange={(event) => setChapterKey(event.target.value)}>
        ...
        <div className="card" style={{ marginTop: 20, display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
          <span className="meta">{sources.length} Drive question sets</span>
          <span className="meta">·</span>
          <span className="meta">Offline cache enabled</span>
          <button type="button" className="primaryButton" onClick={() => void startPractice()} disabled={loading} style={{ marginLeft: "auto" }}>
            {loading ? "Loading questions…" : "Start / Reload Practice →"}
          </button>
        </div>
        ...
        {question && !loading && !error && (
          <div className="card" style={{ marginTop: 20 }}>
            ...
            <div className="meta" style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
              <span>{selectedChapter.chapterName}</span><span>Question {index + 1} / {questions.length}</span>
            </div>
            ...
            <div style={{ display: "grid", gap: 10, marginTop: 24 }}>
              {question.options.map((option) => (
                ...
                >
                  <strong>{String.fromCharCode(65 + Number(option.id))}.</strong>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 22 }}>
              {!submitted ? (
                <button type="button" onClick={() => selected !== null && setSubmitted(true)} disabled={selected === null} className="primaryButton" style={{ opacity: selected === null ? .5 : 1 }}>Check answer</button>
              ) : index < questions.length - 1 ? (
                <button type="button" onClick={() => { setIndex((value) => value + 1); setSelected(null); setSubmitted(false); }} className="primaryButton">Next question →</button>
              ) : (
                <button type="button" onClick={() => void startPractice()} className="primaryButton">New practice set ↻</button>
              )}
            </div>
          </div>
        )}
