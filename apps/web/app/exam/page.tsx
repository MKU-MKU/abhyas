...
export default function ExamPage() {
  ...
  if (!question || submitted) return <main className="main"><section style={{ maxWidth: 900, margin: "0 auto" }}><div className="card" style={{ textAlign: "center" }}><p className="eyebrow">Exam complete</p><h1>{score}/{questions.length}</h1><p className="subtitle" style={{ marginInline: "auto" }}>{questions.length ? Math.round((score / questions.length) * 100) : 0}% accuracy · {answered} answered · {questions.length - answered} unanswered · {chapterName}</p><div style={{ display: "flex", justifyContent: "center", gap: 10, marginTop: 18, flexWrap: "wrap" }}><button type="button" className="primaryButton" onClick={() => void startExam()}>New exam ↻</button><Link className="secondaryButton" href={`/practice?chapter=${chapterKey}`}>Practice this chapter →</Link><Link className="secondaryButton" href="/progress">See progress →</Link></div></div><section style={{ marginTop: 20 }}><div className="sectionHeader"><div><p className="eyebrow">Answer review</p><h2 style={{ margin: 0 }}>See every result</h2></div></div><div className="subtopicList">{questions.map((item, i) => { const selected = answers[i]; const isCorrect = selected === item.answer; const correctLabel = item.options.find((option) => option.id === item.answer)?.label ?? item.answer; return <article className="card" key={item.id}><div className="meta">Question {i + 1} · {isCorrect ? "Correct" : selected ? "Incorrect" : "Unanswered"}</div><h3 style={{ marginTop: 8 }}>{item.prompt}</h3>{selected && !isCorrect && <p style={{ marginBottom: 6 }}>Your answer: {item.options.find((option) => option.id === selected)?.label ?? selected}</p>}<p style={{ marginBottom: item.explanation ? 8 : 0 }}><strong>Correct:</strong> {correctLabel}</p>{item.explanation && <p style={{ marginBottom: 0 }}><strong>Explanation:</strong> {item.explanation}</p>}</article>; })}</div></section></section></main>;
