...
export default function ExamPage() {
  ...
  useEffect(() => {
    ...
    setChapterKey(valid);
    void startExam(valid);
    // Initial exam only; controls call startExam directly.
  ...
  if (!question || submitted) return <main className="main"><section className="card" style={{ maxWidth: 760, margin: "60px auto", textAlign: "center" }}><p className="eyebrow">{seconds === 0 && !submitted ? "Time" : "Exam complete"}</p><h1>{score}/{questions.length}</h1><p className="subtitle" style={{ marginInline: "auto" }}>{questions.length ? Math.round((score / questions.length) * 100) : 0}% accuracy · {answered} answered · {questions.length - answered} unanswered · {chapterName}</p><div style={{ display: "flex", justifyContent: "center", gap: 10, marginTop: 18, flexWrap: "wrap" }}><button type="button" className="primaryButton" onClick={() => void startExam()}>New exam ↻</button><Link className="secondaryButton" href={`/practice?chapter=${chapterKey}`}>Practice this chapter →</Link><Link className="secondaryButton" href="/progress">See progress →</Link></div></section></main>;

  return <main className="main"><section style={{ maxWidth: 900, margin: "0 auto" }}>
    <Link className="cardLink" href="/">← Dashboard</Link>
    <div style={{ marginTop: 24, display: "flex", justifyContent: "space-between", gap: 16, alignItems: "end", flexWrap: "wrap" }}><div><p className="eyebrow">Timed examination</p><h1 style={{ marginBottom: 6 }}>{chapterName}</h1><p className="meta">20 questions · 30 minutes · answers are scored when you submit.</p></div><div style={{ minWidth: 280 }}><label className="meta" htmlFor="exam-chapter">Chapter</label><select id="exam-chapter" className="input" value={chapterKey} onChange={(event) => { const value = event.target.value; setChapterKey(value); void startExam(value); }}>{(chapters.length ? chapters : FALLBACK_CHAPTERS.map(([code, name]) => ({ code, name, count: 0 }))).map((chapter) => <option key={chapter.code} value={chapter.code}>{chapter.name}{chapter.count ? ` · ${chapter.count} Q` : ""}</option>)}</select></div></div>
