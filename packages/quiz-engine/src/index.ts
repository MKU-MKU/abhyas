...
export function evaluateAnswer(question: QuizQuestion, attempt: AnswerAttempt): QuestionResult {
  if (attempt.questionId !== question.id) throw new Error("Answer does not belong to the supplied question.");
  if (!Number.isFinite(attempt.elapsedMs) || attempt.elapsedMs < 0) throw new Error("Elapsed time must be a non-negative finite number.");

  const selected = attempt.selectedOptionIndexes;
  const valid = selected.every((index) => Number.isInteger(index) && index >= 0 && index < question.options.length);
...
export function calculateQuizResult(mode: QuizMode, questions: readonly QuizQuestion[], attempts: readonly AnswerAttempt[]): QuizResult {
  ...
  const results = questions.map((question) => {
    ...
    return attempt
      ? evaluateAnswer(question, attempt)
      : { questionId: question.id, correct: false, answered: false, selectedOptionIndexes: [], elapsedMs: 0 } satisfies QuestionResult;
  });
